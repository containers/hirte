...
def exec(ctrl: BluechiControllerMachine, nodes: Dict[str, BluechiAgentMachine]):
    ...
    system_result_test_one = ResultFuture()
    system_thread_wait = threading.Thread(
        target=check_system,
        args=(
            ctrl.bluechi_is_online,
            IMMEDIATE_RETURN_TIMEOUT_MS,
            system_result_test_one,
        ),
    )
    system_thread_wait.start()
    try:
        with Timeout(SLEEP_DURATION, "Timeout while waiting for system."):
            system_thread_wait.join()
    except TimeoutError:
        LOGGER.debug(
            "Timeout reached while waiting for system. Attempting to terminate."
        )
    assert (
        system_result_test_one.result
    ), "System wait command should not produce output when system is running."
    ...
    system_result_test_two = ResultFuture()
    system_thread = threading.Thread(
        target=check_system,
        args=(
            ctrl.bluechi_is_online,
            IMMEDIATE_RETURN_TIMEOUT_MS,
            system_result_test_two,
        ),
    )
    system_thread.start()
    system_thread.join()
    assert (
        not system_result_test_two.result
    ), f"Expected bluechi-is-online system with --wait={IMMEDIATE_RETURN_TIMEOUT_MS} to return an error"
    assert (
        time.time() - start_time > IMMEDIATE_RETURN_TIMEOUT_MS / 1000
    ), "Expected around 5 seconds for bluechi-is-online to exit"
    ...
    start_time = time.time()
    system_result_test_three = ResultFuture()
    system_thread_wait = threading.Thread(
        target=check_system,
        args=(
            ctrl.bluechi_is_online,
            IMMEDIATE_RETURN_TIMEOUT_MS,
            system_result_test_three,
        ),
    )
    system_thread_wait.start()
    system_thread_wait.join()

    assert not system_result_test_three.result, (
        f"Expected bluechi-is-online system with --wait={IMMEDIATE_RETURN_TIMEOUT_MS}"
        f" to return an error"
    )
    assert (
        time.time() - start_time > IMMEDIATE_RETURN_TIMEOUT_MS / 1000
    ), "Expected around 5 seconds for bluechi-is-online to exit"
